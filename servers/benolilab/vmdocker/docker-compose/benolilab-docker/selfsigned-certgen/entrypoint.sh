...
CRON_TEMPLATE_FILE="/app/selfsigned-certgen.cron"
SCRIPT_FILE="/app/scripts/selfsigned-certgen-trigger.sh"
...
for bin in bash supercronic jq openssl; do
...
# Sanity check: main script must exist and be executable
...
if [ "$DISABLE_CRON" != "true" ]; then
    ...
    echo "[entrypoint] - Launching cron scheduler. DRY_RUN is set to $DRY_RUN.  Make sure DRY_RUN is 'false' for the main script to properly execute."
